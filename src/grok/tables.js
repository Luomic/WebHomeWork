...
export const ONBOARDING = ["curious", "happy", "playful", "excited", "listening", "proud", "laughing", "shy"];
export const ONBOARDING_MS = 1200;
export const onboardMood = (n) => (n % 2 === 0 ? "idle" : ONBOARDING[((n - 1) / 2) % ONBOARDING.length] ?? "idle");
...
export const V_T = new Set(["happy", "excited", "proud"]);
export const B_T = new Set(["playful"]);
export const WINK_STATES = new Set(["idle", "happy", "excited", "curious", "playful"]);
export const SHAPE_ZOOM = { blob: 0.92, pebble: 0.96, squircle: 0.84, tablet: 1, wedge: 0.94, hex: 0.94, cloud: 1, teardrop: 1 };
export const VIEW_SCALE = 259 / 229;
export const shapeZoom = (name) => SHAPE_ZOOM[name] ?? 1;
export const poseScale = (name) => shapeZoom(name) * VIEW_SCALE;
export const shapeEyeScale = (name) => SHAPE_ZOOM.blob / shapeZoom(name);
