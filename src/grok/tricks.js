...
export function hopY(hopAt, now) {
  if (hopAt < 0) return 0;
...
export function startTrick(kind, reduce) {
...
export function evalTrick(trick, now) {
...
export function makeSpinTurn(turns = 1, dir = sign()) {
  const s = spring(0);
  s.t = turns * Math.PI * 2 * dir;
  return s;
}

export function spinTurnSettled(s) {
